...
files.forEach(file => {
  ...
  // Pattern 1: AED {variable} -> AED {Number(variable).toFixed(2)}
  ...
  // Pattern 2: AED ${variable} inside backticks -> AED ${Number(variable).toFixed(2)}
  content = content.replace(/AED \$\{([^}]+)\}/g, (match, p1) => {
    if (p1.includes('.toFixed')) return match;
    return 'AED ${Number(' + p1 + ').toFixed(2)}';
  });

  // Pattern 3: static numbers e.g. AED 20, AED 0, AED 300 -> AED 20.00
