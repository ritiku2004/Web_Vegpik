...
const Contact = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccess(true);
      setForm({ subject: '', message: '', name: '', email: '', phone: '' });
    } catch (err) {
  ...
  const getIconWrapperStyle = (key) => {
    if (key === 'whatsapp') return {};
    if (key === 'phone') return { backgroundColor: 'rgba(245, 158, 11, 0.1)', color: '#f59e0b' };
    if (key === 'email') return { backgroundColor: 'rgba(59, 130, 246, 0.1)', color: '#3b82f6' };
    return { backgroundColor: 'rgba(107, 114, 128, 0.1)', color: '#6b7280' };
  };

  // Filter cards to exclude operating hours (displayed separately)
  const contactCards = contactInfoList.filter(item => item.field_key !== 'operating_hours');
  const operatingHours = contactInfoList.find(item => item.field_key === 'operating_hours');
      ...
      <div className={styles.content}>
        ...
        <div className={styles.contactGrid}>
          {contactCards.map((card) => {
            const isExternal = card.action_label && (card.action_label.startsWith('http') || card.action_label.startsWith('tel:') || card.action_label.startsWith('mailto:'));
            const targetAttr = isExternal && card.action_label.startsWith('http') ? { target: '_blank', rel: 'noopener noreferrer' } : {};
            
            return (
              <a 
                key={card.id} 
                href={card.action_label || '#'} 
                {...targetAttr}
                className={`${styles.contactCard} ${card.field_key === 'whatsapp' ? styles.whatsappCard : ''}`}
              >
                <div className={styles.cardIconWrapper} style={getIconWrapperStyle(card.field_key)}>
                  {getCardIcon(card.icon)}
                </div>
                <div className={styles.cardContent}>
                  <div className={styles.cardTitleRow}>
                    <h3 className={styles.cardTitle}>{card.title}</h3>
                    {card.field_key === 'whatsapp' && <span className={styles.instantBadge}>INSTANT</span>}
                  </div>
                  <p className={styles.cardDescription}>{card.description}</p>
                  <span className={styles.cardValue || styles.cardActionLink}>{card.value}</span>
                </div>
        ...
        {operatingHours && (
          <div className={styles.operatingHoursAlert}>
            ...
            <div className={styles.alertContent}>
              <h4 className={styles.alertTitle}>{operatingHours.title}</h4>
          ...
          {success ? (
            ...
            <form onSubmit={handleSubmit} className={styles.messageForm}>
              ...
              <div className={styles.formGroup}>
                <label className={styles.formLabel}>Your Name <span className={styles.required}>*</span></label>
                <input
                  type="text"
              ...
              <div className={styles.formGroup}>
                <label className={styles.formLabel}>Your Email Address <span className={styles.required}>*</span></label>
                <input
                  type="email"
                  className={styles.selectInput}
